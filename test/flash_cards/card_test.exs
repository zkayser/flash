defmodule Flash.CardTest do
  ...
  describe "update/2" do
    test "when review is successful" do
      card = %Card{id: 1}
      expected_query = from(c in Card, where: c.id == ^1, update: [inc: [successes: 1, times_seen: 1]])
      Helpers.assert_query_equal(Card.update(card, true), expected_query)
    end

    test "when review is unsuccessful" do
      card = %Card{id: 1}
      expected_query = from(c in Card, where: c.id == ^1, update: [inc: [failures: 1, times_seen: 1]])
